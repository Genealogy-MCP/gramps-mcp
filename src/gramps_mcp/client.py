...
class GrampsWebAPIClient:
    ...
    def _format_http_error(self, error: httpx.HTTPStatusError) -> str:
        """Convert HTTP error to user-friendly message with URL context."""
        status_code = error.response.status_code
        # Include the URL path for context (strip base URL for brevity)
        url_path = str(error.request.url)
        if self.base_url and url_path.startswith(self.base_url):
